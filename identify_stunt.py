def identify_stunt(row):
    ...
    #         stunt_ids.append('10')
      ...
      # other stunt: 11
          ...
          if (list2[index[0]] == len1-2) & (list2[index[1]] == len1-1):
              if (str(len1) in back) or (len1 in back):
                return(1)       
              else:
                return(2)   
          ...
          # (1,2,3,4,5) -> (1,2,3,5,4), or (0,1,2,3,4) -> (0,2,1,4,3)
          # far right stunt: 2
          if (list2[index[-1]] == 1) & (list2[index[-2]] == 0):
              if ("1" in back) or (1 in back):
                return(3)
              else:
                return(4)

          # (1,2,3,4,5) -> (3,2,1,4,5), or (0,1,2,3,4) -> (2,1,0,3,4)       4,3,2,1,0        
          # far left-to-mid stunt: 3
          if (len1 > 2) & (list2[index[0]] in range(1,3)) & (list2[index[2]] == len1-1):
            return 7
    #           if 1 in back:
    #             return 7
    #           else:
    #             return 8
          ...
          # (1,2,3,4,5) -> (1,2,5,4,3), or (0,1,2,3,4) -> (0,1,4,3,2)
          # far right-to-mid stunt: 4
          if (len1 > 2) & (list2[index[-1]] in range(len1-3, len1-1)) & (list2[index[-3]] == 0:
                return 8
    ...
    #             return 0
          ...
          # (1,2,3,4,5) -> (1,3,2,4,5), or (0,1,2,3,4) -> (0,2,1,3,4)       4,3,2,1,0
          # left-mid stunt: 5
          if (len1 > 4) & (list2[index[1]] == 2) & (list2[index[2]] == 3):
                return 9
    #           if 2 in back:
    #             stunt_ids.append('5*')
    #           else:
    #             stunt_ids.append('5')
          ...
          # (1,2,3,4,5) -> (1,2,4,3,5), or (0,1,2,3,4) -> (0,1,3,2,4)
          # right-mid stunt: 6
          if (len1 > 4) & (list2[index[-2]] == len1-3) & (list2[index[-3]] == len1-4):
                return 10
    #           if (len1-1) in back:
    # #             stunt_ids.append('6*')
    # #           else:
    # #             stunt_ids.append('6')
          ...
          # If there's only a 4-man rush, left-mid & right-mid are the same
          # So we have a seventh stunt:
          # (1,2,3,4) -> (1,3,2,4), or (0,1,2,3) -> (0,2,1,4)             3,2,1,0        3,1,2,0      4,3,2,1     4,2,3,1
          # mid stunt: 7
          if (len1 == 4) & (list2[index[1]] == 1) & (list2[index[2]] == 2):
              if ("3" in back) or (3 in back):
                return 5  # in this case, * means the right defender in back
              else:
                return 6
          ...
          # (1,2,3,4,5,6) -> (1,5,3,4,2,6), or (0,1,2,3,4,5) -> (0,4,2,3,1,5)
          # left-bigmid: 8
          if (len1 >= 5) & (list2[index[1]] in range(1, len1-3)):
                return 11
    #           if 2 in back:
    #             stunt_ids.append('8*')  # left defender in back
    #           else:
    #             stunt_ids.append('8')
          ...
          # (1,2,3,4,5,6) -> (1,2,5,4,3,6), or (0,1,2,3,4,5) -> (0,1,4,3,2,5)
          # right-bigmid: 9
          if (len1 >= 5) & (list2[index[len1-2]] in range(3, len1-1)):
                return 12
            # if there is an 8 and a 9, then we change it to 10, the bigmid stunt because left-bigmid and right-bigmid could be identical
    #         if stunt_ids:
    #           if (stunt_ids[-1] in range(8,10)):
